...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image, vis_iou_thr)
...
def fast_rcnn_inference_single_image(
    ...
    # 1. Filter results based on detection scores. It can make NMS more efficient by filtering out low-confidence detections.

    filter_mask = scores > score_thresh  # R x K
    # R' x 2. First column contains indices of the R predictions; Second column contains indices of classes.
    ...
    if vis_iou_thr < 1.0:
        boxes, scores, filter_inds = unknown_aware_nms(boxes, scores, filter_inds, iou_thr=vis_iou_thr)
...
class CosineFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(self, *args, scale: int = 20, vis_iou_thr: float = 1.0, **kargs,):
...
class OpenDetFastRCNNOutputLayers(CosineFastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kargs
        ...
        self.register_buffer('queue', torch.zeros(self.num_known_classes, ic_loss_queue_size, ic_loss_out_dim))
        self.register_buffer('queue_label', torch.empty(self.num_known_classes, ic_loss_queue_size).fill_(-1).long())
        self.register_buffer('queue_ptr', torch.zeros(self.num_known_classes, dtype=torch.long))
        '''
    ...
    def forward(self, feats):
        ...
        return scores, proposal_deltas, cls_x
    ...
    def _dequeue_and_enqueue(self, feat, gt_classes, ious, iou_thr=0.7):
        ...
        for i in range(self.num_known_classes):
            ...
            _, sim_inds = F.cosine_similarity(cls_feat[:, None], cls_queue[None, :], dim=-1).mean(dim=1).sort()
            ...
            # 4. in queue
            batch_size = cls_feat.size(0) if ptr + cls_feat.size(0) <= self.ic_loss_queue_size else self.ic_loss_queue_size - ptr
...
class PROSERFastRCNNOutputLayers(CosineFastRCNNOutputLayers):
    ...
    def get_proser_loss(self, scores, gt_classes):
        ...
        mask = torch.arange(num_classes).repeat(num_sample, 1).to(scores.device)
...
class DropoutFastRCNNOutputLayers(CosineFastRCNNOutputLayers):
    ...
    def predict_probs(self, predictions: List[Tuple[torch.Tensor, torch.Tensor]], proposals: List[Instances]):
        """
        Args:
            predictions: return values of :meth:`forward()`.
            proposals (list[Instances]): proposals that match the features that were
